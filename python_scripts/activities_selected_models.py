...
script = open('results/all_activities.txt', 'w')
...
for term in activities.keys():
    ...
    fig4.savefig(workpath + "figures/{}.png".format(question))
    ...
    f = open('results/ALL_RES_{}.txt'.format(question), 'w')
    ...
    f.close()
    with pd.ExcelWriter(workpath + "results/COM_RES_{}.xlsx".format(question)) as writer:
...
with pd.ExcelWriter(workpath + "results/all_activities_params.xlsx") as writer:
