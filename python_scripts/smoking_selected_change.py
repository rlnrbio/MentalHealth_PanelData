...
def change_in_list(liste):
    # returns year before change
    changes = []
    liste = list(liste)
...
recode = {0:2005, 1:2009, 2:2013, 3:2017, 4: 2019}
total_pivot = total_combined.pivot(index = "pid", columns = "syear", values = "ple0081_v2")
stopped_smoking = []
started_smoking = []
...
for row in total_pivot.iterrows():
    ind = row[0]
    vals = row[1]
    changes = change_in_list(vals)
    if 1 in changes:
        pos = changes.index(1)
        year = recode[pos]
        next_year = recode[pos+1]
        
        stopped_smoking.append((ind, year))
        stopped = stopped.append(total_combined[(total_combined["pid"]==ind) & (total_combined["syear"]==year)])
        stopped = stopped.append(total_combined[(total_combined["pid"]==ind) & (total_combined["syear"]==next_year)])

    if -1 in changes:
        pos = changes.index(-1)
        year = recode[pos]
        started_smoking.append((ind, year))
        started = started.append(total_combined[(total_combined["pid"]==ind) & (total_combined["syear"]==year)])
        started = started.append(total_combined[(total_combined["pid"]==ind) & (total_combined["syear"]==next_year)])
...
total_combined = stopped
variable = "stopped_smoking"
...
total_combined = started
variable = "started_smoking"
