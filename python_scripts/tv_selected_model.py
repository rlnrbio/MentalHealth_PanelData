...
tv_data = load_data_long(personal_df, "tv")
tv_data  = tv_data.replace([1,2,3,4,5], [4,3,2,1,0])

fig4, ax4 = plt.subplots( nrows=1, ncols=1 )  # create figure & 1 axis
ax4.hist(tv_data["pli0083"], bins  = 5)
fig4.savefig(workpath + "figures/tv.png")
...
print("tv", np.unique(tv_data["syear"]))


# initial check:
# OLS Regression between Gender, Age and Personality traits:

b5_years = [2009, 2013, 2019]
mcs_years = [2010, 2014, 2020]
wealth_years = [2007, 2012, 2017]

tv_years = [2008, 2013, 2019]
...
for i in range(len(b5_years)):
    ...
    tv_year = tv_data[tv_data["syear"] == tvy].loc[:,["pid", "pli0083"]]
    ...
    combined = pd.merge(combined, tv_year, on=['pid','pid'])
