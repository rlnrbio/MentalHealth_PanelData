...
sm_data = load_data_long(personal_df, "socialmedia")
sm_data  = sm_data.replace([1,2,3,4,5], [4,3,2,1,0])

fig4, ax4 = plt.subplots( nrows=1, ncols=1 )  # create figure & 1 axis
ax4.hist(sm_data["plh0390"], bins  = 5)
fig4.savefig(workpath + "figures/social.png")
...
print("social", np.unique(sm_data["syear"]))
...
for i in range(len(b5_years)):
    ...
    sm_year = sm_data[sm_data["syear"] == smy].loc[:,["pid", "plh0390"]]
    ...
    combined = pd.merge(combined, sm_year, on=['pid','pid'])
